...
def train(args):
    ...
    if args.arch == 'squeezenet':
        model = SqueezeNet(images_ph, num_classes=NUM_CLASSES, mode='train', channels_first=False, sess=sess)#, seed=args.seed)
        model_file_ext = 'npy'
    else:
        model = MobileNetFC(input_tensor=images_ph, num_classes=NUM_CLASSES, sess=sess)
        model_file_ext = 'h5'
    ...
    os.makedirs('traindata/{}/model'.format(args.arch), exist_ok=True)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        # save epoch model
        model.save_weights('traindata/{}/model/{}.{}'.format(args.arch, epoch, model_file_ext))
    sess.close()
    t1 = time.time() # stop cron

    # save training loss curve
    plt.plot(steps, loss_avg_per_sample)
    plt.savefig('traindata/{}/loss.png'.format(args.arch))
    loss_plt_data = dict(loss=loss_avg_per_sample, steps=steps)
    np.save('traindata/{}/loss_plt_data.npy'.format(args.arch), np.array(loss_plt_data))
    return t1 - t0
...
def validate(args):
    ...
    for epoch in range(1, args.num_epochs + 1):
        # load epoch model
        model.load_weights('traindata/{}/model/{}.{}'.format(args.arch, epoch, model_file_ext))
        total_correct = 0
        for step in range(1, num_steps_per_epoch + 1):
            images, labels = sess.run(next_batch_op)
    ...
    plt.savefig('traindata/{}/accuracy.png'.format(args.arch))
    accuracy_plt_data = dict(accuracy=accuracy_per_epoch, epochs=epochs)
    np.save('traindata/{}/accuracy_plt_data.npy'.format(args.arch), np.array(accuracy_plt_data))
    return best_epoch, t1 - t0


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='DeepRec (SIB18) :: Training the network.')
    parser.add_argument(
